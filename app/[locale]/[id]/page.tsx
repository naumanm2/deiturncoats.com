...
export default async function ProductPage({
  ...
  return (
    <Main locale={locale} page={params.id}>
      <ProductView
        id={product.id}
        locale={locale}
...
export async function generateStaticParams() {
  ...
  const contentImports = await Promise.all(
    locales.map((locale) => import(`@/content/${locale}.json`))
  );
